...
public final class AmplFileWritingTest {
    ...
    public void filerClassesWithTabIndent() throws java.io.IOException {
        TypeSpec test = TypeSpec.classBuilder("Test").addField(java.util.Date.class, "madeFreshDate").addMethod(MethodSpec.methodBuilder("main").addModifiers(javax.lang.model.element.Modifier.PUBLIC, javax.lang.model.element.Modifier.STATIC).addParameter(String[].class, "args").addCode("$T.out.println($S);\n", System.class, "Hello World!").build()).build();
        JavaFile.builder("foo", test).indent("\t").build().writeTo(filer);
        java.nio.file.Path fooPath = fsRoot.resolve(fs.getPath("foo", "Test.java"));
        com.google.common.truth.Truth.assertThat(java.nio.file.Files.exists(fooPath)).isTrue();
        String source = new String(java.nio.file.Files.readAllBytes(fooPath));
    ...
    public void fileIsUtf8() throws java.io.IOException {
        ...
        com.google.common.truth.Truth.assertThat(new String(java.nio.file.Files.readAllBytes(fooPath), java.nio.charset.StandardCharsets.UTF_8)).isEqualTo(("" + (((("// Pi\u00f1ata\u00a1\n" + "package foo;\n") + "\n") + "class Taco {\n") + "}\n")));
    }
}
